...
function validate() {
    ...
    let paymentSelected = document.querySelector('#payment').value
    if (paymentSelected == 'credit card') {
        if (nameValidation() && emailValidation() && activitiesValidation() && creditCardValidation() && zipCodeValidation() && cvvValidation()) {
            return true
        } else {
            return false
        }
